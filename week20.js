...
function makeThree() {
	...
	try {
		throw new Error("Попытка-не пытка");
	} catch (error) {
...
function makeTwelve() {
	let age = 12;
	if (age < 18) {
		throw Error('Доступ запрещен для лиц младше 18 лет');
	} else {
...
function makeNineteen() {
	...
	try {
		...
		console.error(error); //Вывод ошибки в консоль
		resultEighteen.textContent = 'Ошибка'; //Вывод '0' при ошибке
		console.error(error);
	}//Вывод ошибки в консоль
	finally {
			console.log('finally19');
		}
	}
...
const resultTwenty = document.getElementById('result20');
	const wordFinally = document.getElementById('result20a');
...
function makeTwenty() {
	try {
		...
		wordFinally.textContent = 'finally20'
	}
}
